...
def publisher(experiment):
    ...
    pub_stiffness = rospy.Publisher('/iiwa/CartesianImpedance_trajectory_controller/set_config', ControllerConfig, queue_size=10)
    pub_wrench = rospy.Publisher('/iiwa/CartesianImpedance_trajectory_controller/set_cartesian_wrench', WrenchStamped, queue_size=10)
    ...
    target_pose_ac = actionlib.SimpleActionClient('/iiwa/cartesian_trajectory_generator/goal_action', TrajectoryAction)
    target_pose_ac.wait_for_server()

    rospy.wait_for_service('/iiwa/iiwa_fk_server')
    srv_fk = rospy.ServiceProxy('/iiwa/iiwa_fk_server', GetFK)
